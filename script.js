...
function generateNameList(characters) {
  characters.forEach (character => {
    const li = document.createElement("li");
    li.textContent = character.name;
...
function ageFilterCharacter(characters, age) {
  for (let i = 0; i < characters.length; i++) {
...
ageFilterCharacter(characters,90);
...
function ageFilterErrorHandling(characters, errorList, errorMessage, age) {

characters.filter(character => character.age < age).forEach(element => {
  try {
    if (!element.hasOwnProperty('name')) {
      throw new Error(`Character under ${age} year olds with ID ${element.id} is missing 'Name'.`)
    }
    const li = document.createElement("li");
    ...
    errormsg.className = "error-messages";
    errormsg.textContent = error.message;
    errorMsg.appendChild(errormsg);
  }
});
}
...
ageFilterErrorHandling (characters, "error-handling-list", "error-messages", 40);
ageFilterErrorHandling(brokenCharacters, "broken-array-list", "broken-array-errors", 40);